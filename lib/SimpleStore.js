...
var SimpleStore = (function (_EventEmitter) {
  ...
  _createClass(SimpleStore, [{
    ...
    value: function init() {
      ...
      if (!this.__loaded) {
        if (this.__sync) {
          this.__sync.context(_context).fetchAll(this.__loadData.bind(this));
        } else {
    ...
    value: function create() {
      ...
      if (this.__sync) {
        this.__sync.context(context).create(record, this.__add.bind(this));
      } else {
        this.__add(record);
      }
    }
    ...
    value: function update() {
      ...
      var context = _ref3$context === undefined ? null : _ref3$context;
    ...
    value: function _delete() {
      ...
      if (record.get("__cid") && record.get("id")) {
        if (this.__sync) {
          this.__sync.context(context).update(record, this.__remove.bind(this));
        } else {
